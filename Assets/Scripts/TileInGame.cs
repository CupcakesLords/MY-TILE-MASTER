...
public class TileInGame : MonoBehaviour
{
    ...
    private int UponOtherTileSelected(GameObject id, int pivot)
    {
        if(id == gameObject && selected)
        {
            ...
            mover = MoveToCoroutine(transform, new Vector3(BoardManager.instance.xBar + pivot, BoardManager.instance.yBar, 0), 0.5f);
    ...
    private int UponMatchRearrange(GameObject identity, int pos)
    {
        if(selected && identity == gameObject)
        {
            ...
            mover = MoveToCoroutine(transform, new Vector3(BoardManager.instance.xBar + pos, BoardManager.instance.yBar, 0), 0.5f);
            StartCoroutine(mover);
        }
        return 0;
    }
